...
class _NavigationPageState extends State<NavigationPage>
  ...
  // ... (Tüm renkler ve state değişkenleri aynı kalır) ...
  ...
  void initState() {
    ...
    // <<< DEĞİŞTİRİLDİ: Video yükleme mantığı önbelleği kontrol ediyor >>>
  ...
  void dispose() {
    BleRouter().start();
    _controller.dispose(); // <<< Bu controller'ın dispose edilmesi önemli
  ...
  // <<< YENİ METOT: Önbelleği kontrol eder, yoksa sıfırdan yükler >>>
  Future<void> _initializeVideoWithCache() async {
    // 1. Önbellekten almayı dene
    final cachedController = VideoCacheService().getController(_routeVideo.name);

    if (cachedController != null) {
      // 2. Önbellekte bulunduysa:
      _controller = cachedController;
      if (mounted) {
        setState(() {
          _isControllerReady = true;
          // Serviste zaten ayarlandı ama tekrar kontrol edelim
          _controller.setLooping(false);
          _controller.setVolume(0.0); // <<< SES KALICI OLARAK KAPATILDI
          // _isMuted = true; // <<< KALDIRILDI
        });
      }
    } else {
      // 3. Önbellekte yoksa (veya hata olduysa): Sıfırdan yükle
      _initializeVideoPlayer();
    }
  }

  // <<< DEĞİŞTİRİLDİ: Bu metot artık sadece önbellekte yoksa çağrılıyor >>>
  ...
  // (_findRouteVideo, _startNavigation, _playPauseVideo, _stopVideo, vb.)
